    ...
    case 'list-item-add':
      return _ref => {
        ...
        if ('itemValue' in action) {
          ...
          value = value.push(type === 'object' ? OrderedMap() : List.isList(items) || type === 'array' ? List() : type === 'string' ? '' : type === 'null' ? null : undefined);
        }
    ...
    case 'list-item-move':
      return _ref3 => {
        ...
        return _objectSpread({
          ...
          internal: internal.update('internals', (internalInternals = List()) => moveItem(internalInternals.size - 1 < action.toIndex ? internalInternals.set(action.toIndex, undefined) : internalInternals.size - 1 < action.fromIndex ? internalInternals.set(action.fromIndex, undefined) : internalInternals, action.fromIndex, action.toIndex))
        }, r);
      };
    case 'set':
      return action.data;
