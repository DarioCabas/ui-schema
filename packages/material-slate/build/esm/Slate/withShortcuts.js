...
export const withShortcuts = options => editor => {
  ...
  editor.insertText = text => {
    ...
    if (text === ' ' && selection && Range.isCollapsed(selection)) {
      ...
      if (type && editorIsEnabled(options.enableOnly, type) && !(block && block[0].type === pluginOptions.li.type)) {
        ...
        if (type === pluginOptions.ol.type) {
          ...
          Transforms.wrapNodes(editor, list, {
            match: n => !Editor.isEditor(n) && SlateElement.isElement(n) && n.type === pluginOptions.li.type
          });
  ...
  editor.deleteBackward = (...args) => {
    ...
    if (selection && Range.isCollapsed(selection)) {
      ...
      if (match) {
        ...
        const start = Editor.start(editor, path);
        if (!Editor.isEditor(block) && SlateElement.isElement(block) && block.type !== pluginOptions.p.type && Point.equals(selection.anchor, start)) {
          const newProperties = {
          ...
          if (block.type === pluginOptions.li.type) {
            Transforms.unwrapNodes(editor, {
              match: n => !Editor.isEditor(n) && SlateElement.isElement(n) && (n.type === pluginOptions.ul.type || n.type === pluginOptions.ol.type),
              split: true
            });
          }
          return;
